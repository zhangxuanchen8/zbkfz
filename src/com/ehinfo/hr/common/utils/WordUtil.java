...
public class WordUtil {
	...
	// 获取一个字符串的拼音码
	public static String trans2PyCode(String oriStr) {
		JSONObject o = JSONObject.fromObject(getJsonPyWbCode(oriStr));
		return o.get("py").toString();
	}
	//获取一个字符串的五笔码
	public static String trans2WbCode(String str) {
		JSONObject o = JSONObject.fromObject(getJsonPyWbCode(str));
		return o.get("wb").toString();
	}
	 ...
	 *将字符串转换成五笔码与拼音码
	 *
	 *@return MAP格式，key:wb,py
	 * */
	public static Map<String, String> GetConvertCode(String str){
		String py ="";
		String wb ="";
		List<BaseWordDict> bwss = null;
		List<String> cnString = null;
		try {   //一次性取出表里的数据，再处理
			cnString = new ArrayList<>();
			for (int i = 0; i < str.length(); i++) {
				String cnChar = String.valueOf(str.charAt(i));
				cnString.add(cnChar);
			}
			bwss = wordUtil.service.findPyWb(cnString);
		} catch (Exception e) {
			e.printStackTrace();
		} 
		//处理开始
		for (int i = 0; i < str.length(); i++) {
			String cnChar = String.valueOf(str.charAt(i));
			int flag = 0;
			for (BaseWordDict bw : bwss) {
				if(cnChar.equals(bw.getCnchar())){
					py += bw.getPychar();
					wb += bw.getWbchar();
					flag = 1;
					break;
			...
			if(flag == 0){//取出的数据里没有
				py += trans2PyCodeLocal(cnChar);
				wb += trans2WbCodeLocal(cnChar);
			}
		}
	 ...
	 * 获取字符串的拼音码和五笔码
	 * @param str  词语
	 * @return json格式字符串 {'py':'xxxx','wb':'xxxx'}
	 */
	public static String getJsonPyWbCode(String str) {
		Map<String,String> m  = GetConvertCode(str);
		// 创建JSON格式
	...
	// 获取一个字符串的拼音码本地的，不是从数据库的
	public static String trans2PyCodeLocal(String oriStr) {
	...
	//本地获取一个字符串的五笔码本地的，不是从数据库的
	public static String trans2WbCodeLocal(String str) {
