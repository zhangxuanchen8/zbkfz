...
/*$(function() {
	...
	JY={Object:{notNull:function(obj) {//判断某对象不为空..返回true 否则 false
		...
		Dict:{//ids 对应id值(多个逗号分隔).keys 对应key值(多个逗号分隔).type(可选)1.请选择，2.自定义数组。默认不填.dfstr (可选)自定义数组, 3用value和 selected作对比设置默认值
				setSelect : function(ids, keys, type, dfstr, selected) {
						$.ajax({
							type : 'POST',
							...
							success : function(data, textStatus) {
								if (data.res == 1) {
					        		...
					        		else if(type==4){for(var i=0;i<idss.length;i++){name=map[idss[i]].name;opts="<option value=''></option>";$.each(map[idss[i]].items,function(n,v) {opts+="<option value='"+v.value+"'";if(v.value==selected){opts+=" selected='selected'"}opts+=">"+v.name+"</option>";});$("#"+idss[i]+" select").append(opts);$("#"+idss[i]).trigger("liszt:updated");};}
									else {
										for (var i = 0; i < idss.length; i++) {
		...
		setSelectuseoption : function(ids, keys, type, dfstr, selected,dkey) {
			$.ajax({
				type : 'POST',
				url : jypath + '/backstage/dataDict/getDictSelectuseOption',
				data : {
					ids : ids,
					keys : keys,
					dkey : dkey
		...
		Page:{//跳转分页
			  ...
			  setPagenoselect:function(formId,pageId,pagesize,pagenum,totalCount,fun){
				  if(totalCount>0){
					  var pageul = $("#"+pageId+" ul"),html="";
					  pageul.empty();
					  var leng = parseInt((totalCount - 1)/pagesize)+1;
		...
		Model:{//loading框
			...
			edit:function(id,title,savefn,cancelfn,width){
				if(width!=""&&width!=null){
					//
					$("#" + id)
					.removeClass('hide')
					.dialog(
							{	
								...
								resizable:true,width:width,modal:true,
								title : "<div class='widget-header'><h4 class='smaller'>"
										+ (JY.Object.notNull(title) ? title
												: "修改") + "</h4></div>",
								title_html : true,
								buttons : [
										{
			...
			zidinyitanchuang:function(id,title,width,height,savefn,cancelfn){
				$("#"+id).removeClass('hide').dialog({resizable:true,width:width,height:height,modal:false,title:"<div class='widget-header' style='height:10px;'><h6 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h6></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i><span id='savebtnstr'>&nbsp;保存<span>","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
			},
			modelwithbtnname:function(id,title,width,height,savefn,savebtnstr,cancelfn){
				$("#"+id).removeClass('hide').dialog({resizable:true,width:width,height:height,modal:false,title:"<div class='widget-header' style='height:10px;'><h6 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h6></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;"+savebtnstr+"","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
			},
			modelnosavebtn:function(id,title,width,height,savefn,savebtnstr,cancelfn){
		...
		synchro : function(form, url, param, fn) {
			var params = form || param || {};
			if (typeof form == 'string') {
				params = $.extend(param || {}, JY.Object
						.serialize($("#" + form)), {
...
function autoheight(t,temp){
	...
	$("#"+t).css({"height":parseInt(winHeight)});
}
