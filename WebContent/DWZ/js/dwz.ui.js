...
/*    if ($.fn.xheditor) {
    ...
    if ($.fn.ajaxTodo100) $("a[target=ajaxTodo100]", $p).ajaxTodo100();
    ...
    $('.j-accordion', $p).each(function() {
        ...
        var initAccordion = function(hBox, height) {
            ...
            var h1        = $pheader.outerHeight(true);
            $this.css('height', height);
            h1=h1+2;
            height = height-1 - (h1 * $pheader.length) - (5 * ($pheader.length-1));
    ...
    $('.j-selectzTree', $p).each(function() {
        ...
        var setPosition = function($box) {
            ...
            if (height!='auto'&&($(window).height()-top-height-h)<0){
            	$box.css({top:(top - height), left:left, 'max-height':(maxHeight+height)});
            }else{
    ...
    //selectOne-dialog
	 ...
	 //select_navTab
	 $("a[target=selectOneNavTab]", $p).each(function(){
	        $(this).click(function(event){
	            var $this = $(this);
	            $this.parent().addClass('active').siblings().removeClass('active');
	            var title = $this.attr("title") || $this.text();
	            var targetType = $this.attr("targetType");
	            var tabid = $this.attr("rel") || "ids";
	            var ids = "";
	 		    var $box = targetType == "dialog" ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
	 		    $box.find("input:checked").filter("[name='"+tabid+"']").each(function(i){
	 				var val = $(this).val();
	 				ids += i==0 ? val : ","+val;
	 		    });
	 		    if (!ids) {
